...
import com.zlk.bigdemo.android.volley.Cache;
import com.zlk.bigdemo.android.volley.VolleyLog;
import com.zlk.bigdemo.freeza.BaseApplication;
import com.zlk.bigdemo.freeza.util.FileUtils;
import com.zlk.bigdemo.freeza.util.ImageUtils;
...
public class ImageDiskCache implements Cache {
	...
	public ImageDiskCache(Context context) {
		this(new File(FileUtils.getCachePath(context), IMAGE_CACHE_DIR));
	}
	...
	public synchronized Entry get(String key) {
		...
		if (key.startsWith("local")) {
			String url = key.substring(key.indexOf(":")+1,key.length());
			String realKey = getFileForKey(key).getAbsolutePath();
			bytes = ImageUtils.getBytes(realKey);
			if(bytes == null){
				String realUrl = url.substring(0, url.lastIndexOf("_"));
				String width = url.substring(url.lastIndexOf("_")+1, url.length());
				File file = ThumbnailUtils.compressAndRotateToBitmapThumbFile(BaseApplication.getInstance(), realUrl, realKey, Integer.valueOf(width), Integer.valueOf(width));
				if(file != null){
					bytes = ImageUtils.getBytes(file.getAbsolutePath());
				}
			}
		} else if(!key.startsWith("http")){
			bytes = ImageUtils.getBytes(key);
		}else {
